...
public class CriminalModel {
	...
	/*
	 * public CrimnalBean findByLogin(String crimnal_name) throws Exception {
	 * 
	 * CrimnalBean bean = null; Connection conn = null; try { conn =
	 * JDBCDataSource.getConnection(); } catch (SQLException e) {
	 * e.printStackTrace(); } try { PreparedStatement ps =
	 * conn.prepareStatement("SELECT * FROM crimnalinfo WHERE crimnal_name=?");
	 * ps.setString(1, crimnal_name); ResultSet rs = ps.executeQuery(); while
	 * (rs.next()) { bean = new CrimnalBean(); bean.setId(rs.getLong(1));
	 * bean.setCrimnal_name(rs.getString(2));
	 * bean.setCrimnal_address(rs.getString(3)); bean.setGender(rs.getString(4));
	 * bean.setCrime_time(rs.getTime(5));
	 * bean.setPolicestationname(rs.getString(6)); bean.setImage(rs.getBlob(7));
	 * bean.setState(rs.getString(8)); bean.setCreatedby(rs.getString(9));
	 * bean.setModifiedby(rs.getString(10));
	 * bean.setCreatedatetime(rs.getTimestamp(11));
	 * bean.setModifieddatetime(rs.getTimestamp(12)); } rs.close(); } catch
	...
	/*
	 * public CrimnalBean Authenticate(String Crimnal_name, String Crimnal_address)
	 * throws Exception { CrimnalBean bean = null; Connection conn = null;
	 * 
	 * conn = JDBCDataSource.getConnection(); PreparedStatement ps = conn
	 * .prepareStatement("SELECT * FROM crimnalinfo WHERE crimnal_name =? AND crimnal_address =?"
	 * ); ps.setString(1, Crimnal_name); ps.setString(2, Crimnal_address); ResultSet
	 * rs = ps.executeQuery(); while (rs.next()) { bean = new CrimnalBean();
	 * bean.setId(rs.getLong(1)); bean.setCrimnal_name(rs.getString(2));
	 * bean.setCrimnal_address(rs.getString(3)); bean.setGender(rs.getString(4));
	 * bean.setCrime_time(rs.getTime(5));
	 * bean.setPolicestationname(rs.getString(6)); bean.setImage(rs.getBlob(7));
	 * bean.setState(rs.getString(8)); bean.setCreatedby(rs.getString(9));
	 * bean.setModifiedby(rs.getString(10));
	 * bean.setCreatedatetime(rs.getTimestamp(11));
	 * bean.setModifieddatetime(rs.getTimestamp(12)); } return bean; }
	 */
